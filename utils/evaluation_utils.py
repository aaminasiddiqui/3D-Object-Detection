...
def cvt_box_2_polygon(box):
    """
    Converts a bounding box into a Shapely Polygon while ensuring it is valid.
    Handles cases of invalid geometries, such as non-closed loops or degenerate points.

    Args:
        box (ndarray): Array of points representing the box.

    Returns:
        Polygon or None: Valid Shapely Polygon object or None if input is invalid.
    """
    try:
        # Step 1: Extract points and ensure they form a valid loop
        points = [(box[i, 0], box[i, 1]) for i in range(len(box))]

        # Check for minimal number of points to form a polygon
        if len(points) < 4:
            raise ValueError("Invalid box: A polygon requires at least 4 points (including closing point).")

        # Ensure the polygon is closed (first point == last point)
        if points[0] != points[-1]:
            points.append(points[0])  # Automatically close the loop

        # Step 2: Create the polygon
        polygon = Polygon(points)

        # Step 3: Validate and fix geometry issues if necessary
        if not polygon.is_valid:
            polygon = polygon.buffer(0)  # Fix invalid geometries (e.g., self-intersections)

        return polygon

    except Exception as e:
        # Log or handle the error gracefully
        print(f"Error in cvt_box_2_polygon: {e}")
        print(f"Problematic box: {box}")
        return None
...
'''def iou_rotated_single_vs_multi_boxes_cpu(single_box, multi_boxes):
    """
    Compute IoU between a single rotated box and multiple rotated boxes.
    :param single_box: Numpy array of shape (6,) [x, y, w, l, im, re]
    :param multi_boxes: Numpy array of shape (N, 6) [x, y, w, l, im, re]
    :return: Tensor of IoUs of shape (N,)
    """

    # Validate inputs
    assert not np.any(np.isnan(single_box)), "Invalid values in single_box"
    assert not np.any(np.isnan(multi_boxes)), "Invalid values in multi_boxes"
    assert not np.any(np.isinf(single_box)), "Infinite values in single_box"
    assert not np.any(np.isinf(multi_boxes)), "Infinite values in multi_boxes"

    # Single box properties
    s_x, s_y, s_w, s_l, s_im, s_re = single_box
    s_area = s_w * s_l
    s_yaw = np.arctan2(s_im, s_re)
    s_conners = bev_utils.get_corners(s_x, s_y, s_w, s_l, s_yaw)
    ...
    ious = []
    for m_idx in range(multi_boxes.shape[0]):
        if s_polygon.is_empty or m_boxes_polygons[m_idx].is_empty:
            ious.append(0.0)  # No IoU if invalid
            print(f"Invalid polygon at index {m_idx}")
            continue
...
def post_processing_v2(prediction, conf_thresh=0.95, nms_thresh=0.4):
    """
    Removes detections with lower object confidence scores than 'conf_thresh'
    and performs Non-Maximum Suppression (NMS) to further filter detections.
    Returns detections with shape:
        (x, y, w, l, im, re, object_conf, class_score, class_pred)
    """
    output = [None for _ in range(len(prediction))]  # Initialize output for all images

    for image_i, image_pred in enumerate(prediction):
        # Filter out detections with confidence below the threshold
        image_pred = image_pred[image_pred[:, 6] >= conf_thresh]

        # Skip if no detections remain
        if image_pred.size(0) == 0:
            continue

        # Compute object confidence × class confidence scores
        class_confs, class_preds = image_pred[:, 7:].max(dim=1, keepdim=True)
        scores = image_pred[:, 6] * class_confs.squeeze()
        #print(f'class_cons and class_preds {class_confs.shape} {class_preds.shape}')

        # Sort detections by descending score
        sorted_indices = scores.argsort(descending=True)
        image_pred = image_pred[sorted_indices]
        class_confs = class_confs[sorted_indices]
        class_preds = class_preds[sorted_indices]

        # Prepare detection tensor
        detections = torch.cat((image_pred[:, :7].float(), class_confs.float(), class_preds.float()), dim=1)
        detections = detections[~torch.isnan(detections).any(dim=1)]
        #print(f'detection is {detections} and it shape is {detections.shape}')

        # Perform Non-Maximum Suppression (NMS)
        keep_boxes = []
        iters = 0
        while detections.size(0):
            # Compute IoU for the top-scoring box with the rest
            try:
                iou_scores = iou_rotated_single_vs_multi_boxes_cpu(detections[0, :6], detections[:, :6])
            except Exception as e:
                print(f"Error in IoU computation: {e}")
                break

            # Identify boxes with large overlap and matching class labels
            large_overlap = iou_scores > nms_thresh
            label_match = detections[0, -1] == detections[:, -1]

            # Combine overlap and label match conditions
            invalid_mask = large_overlap & label_match

            # Merge overlapping boxes by weighted average (confidence as weight)
            weights = detections[invalid_mask, 6:7]
            detections[0, :6] = (weights * detections[invalid_mask, :6]).sum(0) / weights.sum()

            # Save the merged box and remove invalid boxes
        ...
        # Stack all kept boxes for the current image
        if len(keep_boxes) > 0:
            output[image_i] = torch.stack(keep_boxes)
