...
def main():
    ...
    if configs.multiprocessing_distributed:
        configs.world_size = 1 #configs.ngpus_per_node * configs.world_size
        mp.spawn(main_worker, nprocs= 1, args=(configs,)) #nprocs= configs.ngpus_per_node
    else:
...
def main_worker(gpu_idx, configs):
    ...
    if torch.cuda.is_available():
        ...
        device = torch.device(f'cuda:{configs.gpu_idx}')
    else:
    ...
    if configs.distributed:
        ...
        if configs.multiprocessing_distributed:
            ...
            configs.rank = configs.rank * 0#configs.ngpus_per_node + gpu_idx
