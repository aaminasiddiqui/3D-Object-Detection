...
def make_data_parallel(model, configs):
    if configs.distributed:
        ...
        if torch.cuda.is_available() and configs.gpu_idx is not None:
            ...
            # Adjust batch size and workers for distributed training
            configs.batch_size = int(configs.batch_size / configs.ngpus_per_node)
            configs.num_workers = int((configs.num_workers + configs.ngpus_per_node - 1) / configs.ngpus_per_node)

            # Initialize DistributedDataParallel with specified device
            model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[configs.gpu_idx])

        elif not torch.cuda.is_available() or configs.gpu_idx is None:
            # Fallback to CPU if CUDA is not available or gpu_idx is None
            model.to('cpu')  # Move the model to CPU if CUDA isn't available

            # For distributed training, no need to specify device_ids when running on CPU
            model = torch.nn.parallel.DistributedDataParallel(model)

        else:
            # In case CUDA is available but gpu_idx is None, you can still set it for the available GPU
            model.cuda()  # Move model to the default GPU
            model = torch.nn.parallel.DistributedDataParallel(model)
